...
<?php if ($group_disabled) {
    ...
    {
        global $new_multiple_value,$provider,$event_date,$duration,$recurrspec,$starttime,$endtime,$locationspec,$speciality;
        ...
        // ====================================
            ...
            $args['endtime'] = $endtime;
            $args['locationspec'] = $locationspec;
            //$args['form_speciality'] = $speciality;
    ...
    function DOBandEncounter($pc_eid)
    {
        ...
        if (!empty($_POST['form_pid'])) {
            ...
            if ((int)$GLOBALS['auto_create_new_encounters']
                ...
                && $is_checkin == '1'
                && !$is_tracker) {
                $encounter = todaysEncounterCheck($_POST['form_pid'], $event_date, $_POST['form_comments'], $_POST['facility'], $_POST['billing_facility'], $_POST['form_provider'], $_POST['form_category'], $_POST['form_speciality'], false);
                if ($encounter) {
        ...
        if (!empty($_POST['form_gid'])) {
            ...
            if ($GLOBALS['auto_create_new_encounters'] && $event_date == date('Y-m-d') && $_POST['form_apptstatus'] == '=') {
                $encounter = todaysTherapyGroupEncounterCheck($_POST['form_gid'], $event_date, $_POST['form_comments'], $_POST['facility'], $_POST['billing_facility'], $_POST['form_provider'], $_POST['form_category'], $_POST['form_speciality'], false, $pc_eid);
                if ($encounter) {
    ...
    if ($_POST['form_action'] == "save") {
        ...
        if ($eid) {
            ...
            if ($GLOBALS['select_multi_providers'] && $row['pc_multiple']) {
                ...
                $speciality = $_POST['form_speciality'];
                ...
                if ($_POST['recurr_affect'] == 'current') {
                    ...
                    foreach ($_POST['form_provider'] as $provider) {
                        ...
                        "pc_speciality = '" . add_escape_custom($_POST['form_speciality']) . "', " .
...
"FROM openemr_postcalendar_categories where pc_active = 1 ORDER BY pc_seq"); 
//$catoptions1="<option value='Family Medicine Consultation'>Family Medicine Consultation</option><option value='Dental'>Dental</option><option value='Cardiology'>Cardiology</option>";
$catoptions = "";
...
<?php
//Display speciality field

$cres1 = sqlStatement("SELECT list_id,title,activity " .
"FROM list_options where  list_id='speciality' AND activity='1' ORDER BY option_id"); 
$catoptions1 ="";
while ($crow1 = sqlFetchArray($cres1)) {

    $catoptions1 .= "<option>".$crow1['title']."</option>";
} 
...
<?php require($GLOBALS['srcdir'] . "/restoreSession.php"); ?>
 ...
 //Add Speciality field into the form with drop down
 ...
 function dateChanged() {
  ...
  if (si < opts.length) f.form_repeat_type.selectedIndex = si;
    ...
    function find_available(extra) {
        ...
        <?php } else {?>
            s = document.forms[0].form_provider.options[s.
            ].value;
...
<!-- Add new field speciality    -->
        <tr>
        <td width='1%' nowrap>
            <b><?php echo xlt('Speciality'); ?>:</b>
        </td>
        <td nowrap>
            <select class='input-sm' name='form_speciality' onchange='set_speciality()' style='width:100%'>
                <?php echo $catoptions1 ?>
            </select>
        </td>
        </tr>
<!-- End speciality -->
