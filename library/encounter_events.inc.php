...
function update_event($eid)
{
  ...
  // no recurr specs, this is used for adding a new non-recurring event
    ...
    // this event is forced to NOT REPEAT
        ...
        $args['form_speciality']=$origEvent['pc_speciality'];
...
function check_event_exist($eid)
{
    ...
    $origEvent = sqlStatement(
        ...
        "and pc_pid=? and pc_eventDate=? and pc_startTime=? and pc_endTime=? and pc_facility=? and pc_billing_location=? and pc_speciality=?",
        array($eid,$pc_catid,$pc_aid,$pc_pid,$pc_eventDate,$pc_startTime,$pc_endTime,$pc_facility,$pc_billing_location,$pc_speciality)
    );
...
function InsertEvent($args, $from = 'general')
{
    ...
    if ($from == 'general') {
        $pc_eid = sqlInsert(
            ...
            "pc_apptstatus, pc_prefcatid, pc_location, pc_eventstatus, pc_sharing, pc_facility,pc_billing_location,pc_room,pc_speciality " .
            ") VALUES (?,?,?,?,?,?,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,1,1,?,?,?,?)",
            ...
            $args['locationspec'],(int)$args['facility'],(int)$args['billing_facility'],$form_room,$args['form_speciality'])
        );
        ...
        sqlStatement(
            ...
            "pc_recurrspec, pc_startTime, pc_endTime, pc_alldayevent, " .
            "pc_apptstatus, pc_prefcatid, pc_location, pc_eventstatus, pc_sharing, pc_facility,pc_billing_location,pc_speciality " .
            ") VALUES (?,?,?,?,?,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            array($args['form_category'],$args['new_multiple_value'],$args['form_provider'],$form_pid,$args['form_title'],
                ...
                $args['starttime'],$args['endtime'],$args['form_allday'],$args['form_apptstatus'],$args['form_prefcat'], $args['locationspec'],$args['form_speciality'],
            1,
            1,
            (int)$args['facility'],
